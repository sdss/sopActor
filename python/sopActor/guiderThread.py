...
def guider_start(cmd, replyQueue, actorState, start, expTime, clearCorrections, force,
                 oneExposure):
    ...
    if clearCorrections:
        for corr in ('axes', 'scale', 'focus'):
            cmdVar = actorState.actor.cmdr.call(
                actor='guider', forUserCmd=cmd, cmdStr=('%s off' % (corr)), keyVars=[], timeLim=3)
            if cmdVar.didFail:
    ...
    cmdStr = '%s %s %s %s' % (('on' if start else 'off'), time_text(expTime), force, oneExposure)
    cmdVar = actorState.actor.cmdr.call(
        actor='guider', forUserCmd=cmd, cmdStr=cmdStr, keyVars=[], timeLim=timeLim)
    ...
    while True:
        try:
            ...
            if msg.type == Msg.EXIT:
                ...
                msg.cmd.respond(
                    "text=\"%s guiding on %s\"" % (('enabling'
                                                    if msg.on else 'disabling'), msg.what))
                ...
                if cmdVar.didFail:
                    msg.cmd.error(
                        'text=%s' % qstr('Failed to set guider %s %s.' % (msg.what, 'on'
                                                                          if msg.on else 'off')))
                ...
                oneExposure = 'oneExposure' if (hasattr(msg, 'oneExposure') and
                                                msg.oneExposure) else ''
                clearCorrections = 'clearCorrections' if (hasattr(msg, 'clearCorrections') and
                                                          msg.clearCorrections) else ''
                start = msg.on

                guider_start(msg.cmd, msg.replyQueue, actorState, start, expTime, clearCorrections,
                             force, oneExposure)
