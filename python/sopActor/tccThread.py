...
def mcp_semaphore_ok(cmd, actorState):
    ...
    if (sem != 'TCC:0:0') and (sem != '') and (sem != 'None') and (sem != None):
        ...
        cmd.error(
            'text="If you are not the owner, confirm that you can steal it from them, then issue: mcp sem.steal"'
        )
        return False
...
def axis_init(cmd, actorState, replyQueue):
    ...
    if check_stop_in(actorState):
        ...
        if check_stop_in(actorState):
            cmd.error(
                'text="Cannot tcc axis init because of bad axis status: Check stop buttons on Interlocks panel."'
            )
            replyQueue.put(Msg.REPLY, cmd=cmd, success=False)
            return
    ...
    if sem == 'TCC:0:0' and axes_are_clear(actorState):
        ...
        if below_alt_limit(actorState):
            cmd.warn(
                'text="Altitude below interlock limit! Only initializing altitude and rotator: cannot move in az."'
            )
...
class SlewHandler(object):
    ...
    def slew(self, cmd, replyQueue):
        ...
        if self.not_ok_to_slew(cmd):
            ...
            cmd.error(
                'text="Trying to start slew with axis states: {}, error codes: {}, and status bits: 0x{:x},0x{:x},0x{:x}"'.
                format(strAxisState, strAxisCode, *axisBits))
            replyQueue.put(Msg.REPLY, cmd=cmd, success=False)
            return
    ...
    def not_ok_to_slew(self, cmd):
        ...
        if below_alt_limit(self.actorState):
            cmd.warn(
                "text='Below alt=18 limit! Ignoring errors in Az, since we cannot move it anyway.'"
            )
    ...
    def do_slew(self, cmd, replyQueue):
        ...
        if cmdVar.didFail:
            ...
            strAxisCode = (','.join(tccModel.keyVarDict['axisErrCode']))
            cmd.error(
                'text="tcc track command failed with axis states: {} and error codes: {}"'.format(
                    strAxisState, strAxisCode))
        ...
        if self.not_ok_to_slew(cmd):
            ...
            cmd.error(
                'text="tcc track command ended with some bad bits set: 0x{:x},0x{:x},0x{:x}"'.
                format(*axisBits))
            cmd.error('text="Failed to complete slew: see TCC messages for details."')
            replyQueue.put(Msg.REPLY, cmd=cmd, success=False)
        else:
