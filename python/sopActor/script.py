...
class Script(object):
    ...
    def loadFromText(self, rawScript):
        ...
        for i, l in enumerate(rawScript):
            ...
            mat = re.search('^(?P<maxTime>\d+\.\d+ +)?(?P<actor>[a-zA-Z][a-zA-Z0-9_]*) +(?P<cmd>.*)', l)
            if not mat:
                raise RuntimeError("failed to parse script line %d: %s" %
                                   (i, l))

            matDict = mat.groupdict()
            maxTime = matDict['maxTime']
            scriptLine = [matDict['actor'],
                          matDict['cmd'],
    ...
    def loadFromScriptFile(self, path):
        try:
            ...
            raise RuntimeError('could not load scriptfile %s: %s' %
                               (path, e))
    ...
    def resolveFilename(self, scriptName):
        path = os.path.join(os.environ['SOPACTOR_DIR'],
                            'scripts',
                            scriptName+'.inp')
        return path
    ...
    def genStatus(self):
        atStep = min(self.atStep,
                     len(self.scriptLines)-1)

        self.cmd.respond('scriptState=%s,%d,%d,%s,%s' %
                         (self.name,
                          atStep+1, len(self.scriptLines),
                          self.state,
                          self.scriptLineAsString(self.scriptLines[atStep])))


    def genStartKeys(self):
        self.genStatus()
        for i, l in enumerate(self.scriptLines):
            self.cmd.respond('scriptLine=%s,%d,%0.1f,%s' %
                             (self.name, i+1,
                              l[2],
                              self.scriptLineAsString(l)))
