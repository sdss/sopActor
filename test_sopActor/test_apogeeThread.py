...
class TestApogeeThread(sopTester.SopThreadTester, unittest.TestCase):
    ...
    def _do_dither(self, dither):
        ...
        self.assertEquals(dither,
                          myGlobals.actorState.models['apogee'].keyVarDict['ditherPosition'][1])
    ...
    def _do_apogee_dither_set(self, nCall, nInfo, nWarn, nErr, expTime, dithers, didFail=False):
        success = apogeeThread.do_apogee_dither_set(self.cmd, myGlobals.actorState, expTime,
                                                    dithers, 'object', '')
        self.assertEqual(success, not didFail)
    ...
    def test_apogee_dither_set_aborting(self):
        ...
        success = apogeeThread.do_apogee_dither_set(self.cmd, myGlobals.actorState, 500, 'AB',
                                                    'object', '')
        self.assertFalse(success)
        self._check_cmd(0, 0, 1, 0, finish=False)
